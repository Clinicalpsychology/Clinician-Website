...
export const listPsychologists = async (req: Request, res: Response): Promise<void> => {
  ...
  if (minRating !== undefined && (!Number.isFinite(minRating) || minRating < 1 || minRating > 5)) {
    sendError(res, 'min_rating must be between 1 and 5', 400, 'VALIDATION_ERROR');
    return;
  }

  const where: string[] = ['u.is_active = TRUE'];
  const bindings: any[] = [];
  const addBinding = (value: unknown): string => {
    bindings.push(value);
    return '?';
  };
  ...
  if (verifiedOnly !== undefined) where.push(`p.license_verified = ${addBinding(verifiedOnly)}`);
  if (acceptingClients !== undefined) where.push(`p.accepting_new_clients = ${addBinding(acceptingClients)}`);
  ...
  const countResult = await database.raw(`SELECT COUNT(*)::int AS total FROM psychologists p JOIN users u ON u.id = p.user_id WHERE ${whereClause}`, bindings);
